    ...
    $scope.$on('$ionicView.enter', function(e) {
        ...
        if(aluno.sugestoes.length != 0 && $scope.sugestoes.length == 0){
            ...
            var popUp= $ionicPopup.confirm({
                    title: 'Sugestão de Grade Disponível',
                    template: 'Deseja prencher sua grade com as nossas sugestões?',
                    cancelText: 'Não',
                    okText: 'Sim'
                }).then(function(res){
                    if(res){
...
.controller('LoginCtrl', function($scope, LoginService, $ionicPopup, $state,$http) {
    ...
    $scope.$on('$ionicView.enter', function(e) {
    	if(escolhas.length){
    		window.localStorage[aluno.matricula] = angular.toJson(escolhas);
  ...
  //Modificação do danilo: "Apenas deixei como comentário para teste no mobile!     
       ...
       $http.get(Url+'/alunos/getAluno/login='+$scope.data.username+'&senha='+$scope.data.senha).success(function(data) {
            ...
            $state.go('app.grade');
            aluno=data;
            var salvo = window.localStorage[aluno.matricula];
            if(salvo){
                escolhas= angular.fromJson(salvo);
            }
...
.controller('SignupCtrl',function($scope, SignupService, $ionicPopup,$state,$http,$ionicModal) {
  ...
  $scope.submit = function(){
    if(!(/^[abcdefghijklmnopqrstuvwxyz]{3,}\b/i.test($scope.data.first_name) && $scope.data.first_name)){
      $ionicPopup.alert({
        title: 'Preencha seu nome!',
        template: 'Nome não pode ter menos do que 3 caracteres ou conter números.'
      });
      return;
    }
    if(!/\b[0-9]{9}\b/ig.test($scope.data.matricula)){
      $ionicPopup.alert({
        title: 'Sua matricula está certa?',
        template: 'Digite somente números. Ela deve ter nove dígitos.'
      });
      return;
    }
    if(!/\b[abcdefghijklmnopqrstuvwxyz0123456789\s]{3,}\b/ig.test($scope.data.login)){
      $ionicPopup.alert({
        title: 'Preencha seu nome de usuário!',
        template: 'Nome de usuário precisa ter pelo menos 3 caracteres.'
      });
      return;
    }
    if(!(/\b[^\t]{6,}\b/ig.test($scope.data.password) && $scope.data.password)){
      $ionicPopup.alert({
        title: 'Senha curta!',
        template: 'Sua senha deve ter ao menos seis caracteres.'
      });
      return;
    }
    if($scope.data.password != $scope.data.password2){
      $ionicPopup.alert({
        ...
        template: 'Redigite sua senha, corretamente.'
      });
      return;
    }
    var result = (MD5($scope.data.password));
